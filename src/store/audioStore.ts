...
export const useAudioStore = create<AudioState>((set, get) => {
    ...
    const initGlobalFX = async () => {
        ...
        // Connect signal path: Bass Channel -> Sidechain Compressor -> Ducking Gain -> Master
        ...
        // Connect modulation path: Follower -> Inverter -> Ducking Gain (with Offset)
        ...
        return { masterBus, channels, reverbBus, delayBus, sidechain, masterFXNodes }
    }

    const initMicChain = (channels: Record<string, Tone.Channel>) => {
        // Initialize Microphone Chain
        const mic = new Tone.UserMedia()
        const micGate = new Tone.Gate(-50)
    ...
    const connectInstruments = (
        ...
        const { bass, lead, drums, pads, harm, sampler } = instruments

        if (bass.outputGain) bass.outputGain.connect(channels['bass'])
        if (lead.outputGain) lead.outputGain.connect(channels['lead'])
        if (drums.output) drums.output.connect(channels['drums'])

        if (drums.kick && masterFXNodes.sidechainNodes) {
            drums.kick.connect(masterFXNodes.sidechainNodes.follower)
        }

        if (pads?.synth) pads.synth.connect(channels['pads'])
        if (harm.outputGain) harm.outputGain.connect(channels['harm'])
        else if (harm.output) harm.output.connect(channels['harm'])
    ...
    return {
        ...
        initialize: async () => {
            ...
            try {
                ...
                const { masterBus, channels, reverbBus, delayBus, sidechain, masterFXNodes } = await initGlobalFX()
                const { mic, micGate, micGain } = initMicChain(channels)
                const instruments = await initInstruments()
                const { bass, fm, lead, drums, pads, harm, sampler } = instruments

                connectInstruments(instruments, channels, masterFXNodes)
