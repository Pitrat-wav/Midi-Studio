...
export interface ArrangementClip {
    ...
    trackId: string
    type: 'midi' | 'audio'
    snapshotId?: number // Required for MIDI
    audioData?: {
        bufferUrl: string
        blobId?: string // Persistent ID for IndexedDB
        originalBpm: number
        warpMode: 'Stretch' | 'Repitch' | 'None'
        peaks?: number[]
    }
...
interface TrackSettings {
    ...
    automationParam: 'volume' | 'filter' | 'cutoff' | 'gain'
    type?: 'instrument' | 'audio'
    isGroup?: boolean
    isCollapsed?: boolean
    parentId?: string | null
    isFrozen?: boolean
    frozenBufferUrl?: string | null
}
...
interface ArrangementState {
    ...
    toggleGroupCollapsed: (trackId: string) => void
    setTrackFrozen: (trackId: string, frozen: boolean, bufferUrl?: string | null) => void

    setSnapResolution: (res: ArrangementState['snapResolution']) => void
...
export const useArrangementStore = create<ArrangementState>()(
    persist(
        (set) => ({
            clips: [
                { id: '1', trackId: 'drums', type: 'midi', snapshotId: 0, startTick: 0, durationTicks: 32 },
                { id: '2', trackId: 'bass', type: 'midi', snapshotId: 0, startTick: 0, durationTicks: 32 },
                { id: '3', trackId: 'drums', type: 'midi', snapshotId: 1, startTick: 32, durationTicks: 32 },
            ],
            markers: [
            ...
            toggleGroupCollapsed: (trackId) => set((state) => ({
                tracks: {
                    ...state.tracks,
                    [trackId]: { ...state.tracks[trackId], isCollapsed: !state.tracks[trackId]?.isCollapsed }
                }
            })),

            setTrackFrozen: (tid, frozen, url = null) => set(s => ({
                tracks: { ...s.tracks, [tid]: { ...s.tracks[tid], isFrozen: frozen, frozenBufferUrl: url } }
            })),

            setSnapResolution: (res) => set({ snapResolution: res }),

            clearArrangement: () => set({ clips: [], markers: [], selectedClipIds: [], automations: {} })
        }),
        {
            name: 'studio-elite-arrangement',
        }
    )
)
