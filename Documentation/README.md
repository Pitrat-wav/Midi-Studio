# 🌌 MIDI Studio Pro: Knowledge Base

Welcome to the official documentation for the Telegram MIDI Studio. This interactive knowledge base provides deep dives into the technical architecture, synthesis engines, and agent history of the project.

## 📂 Navigation Index

### 1. [Technical Core](./1_Technical_Core/ARCH_SYSTEM.md)
Architecture maps, state management, and the Pyodide bridge.
- [Algorithms Guide](./1_Technical_Core/ALGORITHMS_GUIDE.md)
- [Architecture System](./1_Technical_Core/ARCH_SYSTEM.md)
- [Knowledge Base](./1_Technical_Core/KNOWLEDGE_BASE.md)
- [Store Reference](./1_Technical_Core/STORE_REFERENCE.md)
- [Agent Info](./agent_info.md)
- [Project Manifest](./project_manifest.md)

### 2. [Instruments & 3D](./2_Instruments_3D/HARDWARE_REPLICAS.md)
The heart of the studio: Legendary hardware and WebGL rendering.
- [Hardware Replicas (808/909, Sampler Granulator)](./2_Instruments_3D/HARDWARE_REPLICAS.md)
- [WebGL Engine Deep Dive](./2_Instruments_3D/GL_ENGINE_DEEP_DIVE.md)

### 3. [Workflow & I/O](./3_Workflow_IO/MIDI_PROTOCOL.md)
How to extract your music and master the interface.
- [MIDI Protocol & Export](./3_Workflow_IO/MIDI_PROTOCOL.md)
- [Global Hotkeys](./3_Workflow_IO/HOTKEYS.md)

### 4. [Agent History](./4_Agent_History/WALKTHROUGH_ARCHIVE.md)
The evolution of the project seen through the eyes of its creator.
- [Walkthrough Archive](./4_Agent_History/WALKTHROUGH_ARCHIVE.md)
- [Antigravity Thinking Methodology](./4_Agent_History/ANTIGRAVITY_THINKING.md)

### 5. [User Reference](./5_User_Reference/FAQ_EXTENDED.md)
Guides for the end user and general synthesis tips.
- [Extended FAQ](./5_User_Reference/FAQ_EXTENDED.md)

---
**Latest Updates (v3.5.0):**
- 🏔️ AUTHENTIC South Park Theme - based on user references!
- 🌤️ Light blue sky, green mountains, white snow landscape
- 🌲 3D pine trees with flat/cartoon shading
- ❄️ Snowy ground with snowball elements
- 🎨 Complete cartoon aesthetic matching the TV show
- Enhanced Whisk UI with visual themes (v3.4.2)

*Last updated: 2026-01-31 22:15 UTC+5*
